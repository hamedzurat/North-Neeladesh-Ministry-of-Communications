...
Printer_Entry :: struct {
	entry_id: u64,
	text:     string,
}
Call :: struct {
	caller_line, requested_callee_line: u8,
	phase:                              string,
}
Service_Call :: struct {
	service, phase: string,
}
Service_Error_Count :: struct {
	kind:  string,
	count: u32,
}
Shift :: struct {
	number:                                                                              u8,
	phase:                                                                               string,
	active_call_count:                                                                   u8,
	completed_routings, required_service_calls, completed_service_calls, service_errors: u32,
	service_error_counts:                                                                [dynamic]Service_Error_Count,
}
...
REF_LINE_LABELS := [LINE_COUNT]cstring {
	...
	"BAGHA NEWS DESK",
	"KOYAL MARKET DEPOT",
	"NEEL UNIVERSITY PRESS CLUB",
	"SHAPLA APARTMENTS ANNEX",
	"SHONARPARA TOWER",
}

REF_ACTION_LABELS := [ACTION_COUNT]cstring{"PTT / OPERATOR", "POLICE", "EMS", "TAP BRIDGE LISTEN"}
