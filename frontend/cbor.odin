...
state_message_is_complete :: proc(value: cbor.Value) -> bool {
	...
	tap_monitoring := map_get_or(output, "tap_bridge_monitoring")
	if !is_nil(tap_monitoring) && !map_has_keys(tap_monitoring, {"caller_line", "callee_line", "caller_tap_port", "callee_tap_port", "audio_clip"}) do return false
