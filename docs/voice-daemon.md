...
PocketTTS always runs on the CPU. `just voice-setup` downloads every official precomputed English embedding from the PocketTTS repository into `python/.models/` using its original filename. The twelve configured subscriber lines map to the currently selected files; additional downloaded embeddings can be tested directly with `scripts/test_pocket_tts.py`. The embedding files are ignored by Git.
