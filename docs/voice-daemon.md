...
Set these environment variables before running `just voice-daemon`:
...
2. Export the four worker command variables and start the daemon: `just voice-daemon`.
...
For an isolated daemon test, type `ptt` and `release` on its stdin instead of using Odin. This exercises the same provider and UDP output path without requiring the GUI.
