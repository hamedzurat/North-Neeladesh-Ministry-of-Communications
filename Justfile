backend-debug address="127.0.0.1:7878" voice_address="127.0.0.1:7879" debug_address="127.0.0.1:7880":
    NN_VOICE_STT_COMMAND="uv run --project python --no-sync python -m voice_workers.stt" NN_VOICE_DIALOGUE_COMMAND="uv run --project python --no-sync python -m voice_workers.dialogue" NN_VOICE_TTS_COMMAND="uv run --project python --no-sync python -m voice_workers.tts" NN_VOICE_TTS_PERSISTENT=1 cargo run --quiet -p exchange-backend --bin exchange-backend -- --bind {{ address }} --voice-bind {{ voice_address }} --debug-bind {{ debug_address }}
